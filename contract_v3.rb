...
module Contract
  ...
  module ClassMethods
    ...
    def contract(meth_or_hash, contract_hash)
      if meth_or_hash.is_a? Hash
        #defer_contract(meth_or_hash)
    ...
    def define_contract(method, contract_hash)
      ...
      accepts, returns = [ contract_hash.keys.first, contract_hash.values.first ].map {|v| arrify v}
      method_contract = method_contracts[method]
      method_contract.accepts = accepts
      method_contract.returns = returns
    end
    ...
    def contract_reader(meths={})
      attr_reader *(meths.keys)
      meths.each do |meth, clause|
        method meth, :returns => [ clause ]
      end
    end

    def contract_writer(meths={})
      attr_writer *(meths.keys)
      meths.each do |meth, clause|
        method meth, :accepts => [ clause ]
      end
    end

    def contract_accessor(meths={})
      attr_accessor *(meths.keys)
      meths.each do |meth, clause|
        method meth, :accepts => [ clause ], :returns => [ clause ]
      end
    end
    ...
    private
      def add_checked_method(meth_name, options)
        write_inheritable_hash :checked_methods, { 
          meth_name => Method.new(meth_name, options)
        }
      end
